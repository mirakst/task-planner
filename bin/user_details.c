...
/** Resets all variables in the given user struct to 0. 
 * @param user[o] Active user struct. */
...
/** Performs a first time setup if there is no settings file.
 * @param user[o] Active user struct. */
void First_Time_Setup (User *user) {
    Print_Line(1, "First time setup");
    Get_Name(user);
    Set_Available_Hours(user);
    Set_Ignore_Availability(user);
    Set_Data_Type(user);
    Print_Line(0, "");
}

/** Gets user input and assigns it to the username in the given user struct.
 * @param user[i/o] Active user struct. */
...
/** Reset the input array of available hours.
 * @param time[o] The user's current available hours. */
...
/** Gets available hours as user input and assigns it to the given user struct.
 *  @param user[i/o] Active user struct. */
...
/** Prints details from the input user struct.
 * @param user[i] Active user struct. */
...
/** Attempts to save the user details to a file.
 * @param user[i] Active user struct.
 * @return -1 if the file could not be saved, and otherwise 1. */
...
/** Sets ignore_availability in the given user struct according to the given input.
 * @param user[o] Active user struct.  */
void Set_Ignore_Availability(User *user) {
    printf("Suggest tasks outside your available hours? (y/n): ");
    user->ignore_availability = Get_Bool_Input();
}

/** Sets use_emissions in the given user struct according to the given input.
 * @param user[o] Active user struct.  */
void Set_Data_Type(User *user) {
    printf("Suggest tasks based on (1) prices or (2) CO2 emissions: ");
    user->use_emissions = Get_Double_Input(2) - 1;
}
